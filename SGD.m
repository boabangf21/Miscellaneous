function [updates,  state] = SGD(gradients, state)
...
% power_grad=(vhat).^p_1 .* log(vhat );
  %state.b = state.beta2 * state.b + (1 - state.beta2) *  power_grad;
 
updates = state.alpha * mhat ./ ((vhat) + state.epsilon);
